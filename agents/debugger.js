...
export async function debuggerAgent(plan, workDir, buildError, emit) {
  ...
  emit({ type: 'agent_log', agent: 'debugger', text: 'Reading current files...\n' });
  ...
  try {
    ...
    emit({ type: 'agent_log', agent: 'debugger', text: 'Sending to debugger agent...\n' });
    ...
    for (const file of files) {
      ...
      emit({ type: 'file_written', agent: 'debugger', file: file.path });
    }

    emit({ type: 'agent_complete', agent: 'debugger', success: true });
    return { success: true };
  } catch (err) {
    console.error('Debugger agent error:', err);
    emit({ type: 'agent_complete', agent: 'debugger', success: false, error: err.message });
