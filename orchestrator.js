...
export async function runPipeline(plan, sessionId, emit) {
  ...
  if (!coderResult.success) {
    ...
    while (debugAttempt < MAX_DEBUG_RETRIES && !fixed) {
      ...
      try {
        const { stdout, stderr } = await execAsync('npm run build', { cwd: workDir, timeout: 180000 });
    ...
    if (!coderResult.success) {
      emit({ type: 'pipeline_error', error: `Build failed after ${MAX_DEBUG_RETRIES} debug attempts. Last error:\n${coderResult.error || coderResult.buildOutput}` });
      return;
    }
  }
  ...
  emit({ type: 'build_complete', workDir, subdomain });
}
